...
/**
 * Fix foreign key da ddt a ddt_depositi
 * 
...
return new class extends Migration
{
    ...
    public function up(): void
    {
        ...
        foreach ($foreignKeys as $fk) {
            try {
                ...
                // Ignora errori se la foreign key non esiste già stata rimossa
                \Log::warning("Errore rimozione FK {$fk->CONSTRAINT_NAME}: " . $e->getMessage());
            }
        }

        // Verifica se le foreign key verso ddt_depositi esistono già
        $existingFKs = DB::select("
            SELECT CONSTRAINT_NAME 
            FROM information_schema.KEY_COLUMN_USAGE 
            WHERE TABLE_SCHEMA = DATABASE() 
            AND TABLE_NAME = 'conti_deposito' 
            AND REFERENCED_TABLE_NAME = 'ddt_depositi'
            AND COLUMN_NAME IN ('ddt_invio_id', 'ddt_reso_id', 'ddt_rimando_id')
        ");
    ...
    public function down(): void
    {
        ...
        try {
            DB::statement('ALTER TABLE conti_deposito DROP FOREIGN KEY conti_deposito_ddt_invio_id_foreign');
        } catch (\Exception $e) {
            // Ignora se non esiste
        }

        try {
            DB::statement('ALTER TABLE conti_deposito DROP FOREIGN KEY conti_deposito_ddt_reso_id_foreign');
        } catch (\Exception $e) {
            // Ignora se non esiste
        }

        try {
            DB::statement('ALTER TABLE conti_deposito DROP FOREIGN KEY conti_deposito_ddt_rimando_id_foreign');
        } catch (\Exception $e) {
            // Ignora se non esiste
        }
        ...
        try {
            DB::statement('ALTER TABLE conti_deposito ADD CONSTRAINT conti_deposito_ddt_invio_id_foreign FOREIGN KEY (ddt_invio_id) REFERENCES ddt(id) ON DELETE SET NULL');
        } catch (\Exception $e) {
            \Log::warning("Errore ripristino FK ddt_invio_id: " . $e->getMessage());
        }

        try {
            DB::statement('ALTER TABLE conti_deposito ADD CONSTRAINT conti_deposito_ddt_reso_id_foreign FOREIGN KEY (ddt_reso_id) REFERENCES ddt(id) ON DELETE SET NULL');
        } catch (\Exception $e) {
            \Log::warning("Errore ripristino FK ddt_reso_id: " . $e->getMessage());
        }

        try {
            DB::statement('ALTER TABLE conti_deposito ADD CONSTRAINT conti_deposito_ddt_rimando_id_foreign FOREIGN KEY (ddt_rimando_id) REFERENCES ddt(id) ON DELETE SET NULL');
        } catch (\Exception $e) {
            \Log::warning("Errore ripristino FK ddt_rimando_id: " . $e->getMessage());
        }
    }
};
